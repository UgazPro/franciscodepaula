      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {students.map((student, idx) => (
            <div key={idx} className="bg-white rounded-xl shadow-md p-6 text-center border-t-4 border-(--greenColor)">
              <Star className="text-(--greenColor) mx-auto mb-3" size={28} fill="currentColor" />
              <h3 className="text-xl font-bold text-(--darkBlueColor) mb-1">{student.name}</h3>
              <p className="text-gray-500 text-sm">{student.grade}</p>
              <p className="mt-2 text-(--greenColor) font-semibold">{student.honor}</p>
            </div>
          ))}
        </div>
      </div>
